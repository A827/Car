...
(function(){
  ...
  // Image URL cleaner (handles Google Images / Bing redirect links)
  function cleanImageUrl(u){
    if (!u) return u;
    try{
      const url = new URL(u);
      const real = url.searchParams.get('imgurl') || url.searchParams.get('mediaurl') || url.searchParams.get('url');
      return real || u;
    }catch(_){ return u; }
  }
  ...
  function updatePreview(){
    ...
    const titleField = qs('#titleInp');
    const autoTitle = [make, model].filter(Boolean).join(' ');
    const title = (titleField.value || autoTitle || 'Title appears here').trim();
    ...
    const raw = qs('#imgUrl').value.trim();
